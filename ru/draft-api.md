...
Обращение через PUT устанавливает значение параметра.

Для всех запросов обязательным параметром является api-key,
по которому происходит авторизация.

Для /video обязательным параметром также является channel, который
определяет, к какому из потоков идет обращение.
